...
#define TEST_NUM   (ACS_SMMU_HYP_TEST_NUM_BASE + 1)
#define TEST_RULE  "B_SMMU_16, B_SMMU_17"
#define TEST_DESC  "Check Stage2 SMMU functionality       "
...
payload()
{
    ...
    while (num_smmu--)
    {
        if (val_smmu_get_info(SMMU_CTRL_ARCH_MAJOR_REV, num_smmu) == 2) {
            ...
            if (VAL_EXTRACT_BITS(data, 29, 29) == 0) {
            ...
            /* Check Stage2 translation support */
            if (VAL_EXTRACT_BITS(data, 0, 0) == 0) {
                val_print(ACS_PRINT_ERR, "\n\t Stage 2 Translation not supported ", 0);
                val_set_status(index, RESULT_FAIL(TEST_NUM, 1));
                return;
            }
