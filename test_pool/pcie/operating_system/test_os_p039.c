...
#define TEST_DESC  "IE_REG_2: Check i-EP atomicop rule                 "
...
payload(void)
{
  ...
  for (tbl_index = 0; tbl_index < bdf_tbl_ptr->num_entries; tbl_index++)
  {
      ...
      if (dp_type == iEP_EP)
      {
          ...
          {
              val_print(ACS_PRINT_DEBUG, " BDF 0x%x atomicop completer fail", bdf);
              test_fails++;
          }
          ...
          if ((ep_requester_cap) &&
             ((rp_routing_cap == 0) && (rp_requester_cap == 0)))
          {
              val_print(ACS_PRINT_DEBUG, " BDF 0x%x atomicop requester fail", bdf);
              test_fails++;
          }
     }
  }
