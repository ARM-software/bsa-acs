  ...
  )
{
   ...
   */
  bsa_print(ACS_PRINT_DEBUG, L"  No SMMU found behind the RootComplex with segment :%d",
                                                                            RcSegmentNum);
  return 0;
...
pal_iovirt_create_info_table_dt(IOVIRT_INFO_TABLE *IoVirtTable)
{
  ...
  if (offset < 0) {
    bsa_print(ACS_PRINT_DEBUG, L"  PCIE node not found %d\n", offset);
    return;
  }
  ...
  while (offset != -FDT_ERR_NOTFOUND) {
      bsa_print(ACS_PRINT_DEBUG, L"  SUBNODE  offset %x\n", offset);
